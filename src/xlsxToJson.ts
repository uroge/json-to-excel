...
export function xlsxToJson(xlsxFilePath: string) {
  try {
    ...
    const outPath =
      path.basename(xlsxFilePath, path.extname(xlsxFilePath)) +
      '.json';
    fs.writeFileSync(
      outPath,
    ...
    if (err.code === 'ENOENT') {
      console.error(`File not found: ${xlsxFilePath}`);
    } else {
      console.error(`Error processing ${xlsxFilePath}:`, err.message);
    }
  }
}
